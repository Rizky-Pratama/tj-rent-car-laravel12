...
@section('page-subtitle', 'Modify order information and rental details')

@section('content')
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="space-y-6" x-data="orderForm()">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Order Details</h3>
                    <span class="text-sm text-gray-500 dark:text-gray-400">Order: {{ $order->order_number }}</span>
                </div>
                        ...
                        <select name="car_id" id="car_id" required x-model="selectedCar" @change="updateCarDetails()"
                            ...
                            @foreach ($cars as $car)
                                <option value="{{ $car->id }}" data-price="{{ $car->price_per_day }}"
                                    data-name="{{ $car->name }}"
                                    data-image="{{ $car->image ? asset($car->image) : '' }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" required
                            value="{{ old('start_date', $order->start_date) }}" x-model="startDate"
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" id="end_date" required
                            value="{{ old('end_date', $order->end_date) }}" x-model="endDate" @change="calculateTotal()"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors duration-200">
                        @error('end_date')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="in_progress"
                                {{ old('status', $order->status) === 'in_progress' ? 'selected' : '' }}>In Progress
                            </option>
                    ...
                    <div>
                        ...
                        <select name="payment_status" id="payment_status"
                            ...
                            <option value="paid"
                                {{ old('payment_status', $order->payment_status) === 'paid' ? 'selected' : '' }}>Paid
                            </option>
                            <option value="cancelled"
                                {{ old('payment_status', $order->payment_status) === 'cancelled' ? 'selected' : '' }}>
                                Cancelled</option>
                        </select>
    ...
    <script>
        function orderForm() {
            return {
                selectedCar: '{{ old('car_id', $order->car_id) }}',
                startDate: '{{ old('start_date', $order->start_date) }}',
                endDate: '{{ old('end_date', $order->end_date) }}',
                pricePerDay: {{ old('car_id', $order->car_id) ? $order->car->price_per_day : 0 }},
                totalDays: {{ old('total_days', $order->total_days) }},
                totalAmount: {{ old('total_amount', $order->total_amount) }},
                carName: '{{ old('car_id', $order->car_id) ? $order->car->name : '' }}',
                carImage: '{{ old('car_id', $order->car_id) && $order->car->image ? asset($order->car->image) : '' }}',
